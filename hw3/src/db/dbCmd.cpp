...
DBAppendCmd::exec(const string& option)
{
   ...
   if(doRow)
   {
      ...
      {
         if(i > dbtbl.nCols()) break;
         else if(options[i] == "-") r.addData(INT_MAX);
         else
         {
            if(myStr2Int(options[i],num)) r.addData(num);
            else 
            {
   ...
   else
   {
      ...
      for(int i = 1; i < options.size(); i++)
      {
         if(i > dbtbl.nRows()) break;
         else if(options[i] == "-") Coldata.push_back(INT_MAX);
         else 
         {
            if(myStr2Int(options[i],num)) Coldata.push_back(num);
            else 
            {
               cerr << "Error: " << options[i] << " is not a number!!\n";
               return CMD_EXEC_ERROR;
      ...
      while(Coldata.size() < dbtbl.nRows())
      {
         Coldata.push_back(INT_MAX);
      }
      dbtbl.addCol(Coldata);     
   }
...
{  
   ...
   {
        bool doTable = false;
        if (myStrNCmp("-Table", options[0], 2) == 0) doTable = true;
   ...
   else if(options.size() == 2)
   {
        ...
        {
           int r,c;
           if (!checkRowIdx(options[0], r)) return CMD_EXEC_ERROR;
           if (!checkColIdx(options[1], c)) return CMD_EXEC_ERROR;
           if(dbtbl[r][c] == INT_MAX) cout<<"null"<<endl;
           else cout<< dbtbl[r][c]<<endl;
        }
   }
   else
   {
