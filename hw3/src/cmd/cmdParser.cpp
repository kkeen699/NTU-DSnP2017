...
CmdParser::closeDofile()
{
   ...
   if ( _dofile != 0 ) {
     if ( !_dofileStack.empty() ) {
       if(_dofileStack.size() == 1) {_dofileStack.pop(); _dofile = 0;}
       else
       {
        _dofileStack.pop();
        _dofile = _dofileStack.top();
...
CmdParser::parseCmd(string& option)
{
   ...
   if(!e) cerr<< "ILLegal command!! \"" << str << "\"" << endl;
   if(n != string::npos) option = str.substr(n+1);
...
CmdParser::getCmd(string cmd)
{
   ...
   for(CmdMap::iterator iter = _cmdMap.begin(); iter != _cmdMap.end(); iter++)
   {
     ...
     {
       if(!myStrNCmp(cmd, iter->first, (iter->first).size()))
       {
         if(cmd.size() > (iter->first).size())
         {
           cmd = cmd.substr((iter->first).size());
           if(!myStrNCmp((iter->second)->getOptCmd(), cmd, cmd.size()))
              e = iter->second;
         }
         else e = iter->second;
       }
