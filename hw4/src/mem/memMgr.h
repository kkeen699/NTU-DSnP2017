...
#define toSizeT(t)      ((t%SIZE_T) ?((t/SIZE_T)*SIZE_T + SIZE_T):t)   // TODO
//
...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
      t = toSizeT(t);
      if(t > getRemainSize()) return false;
      else{
        ret = (T*)_ptr;
        _ptr = _ptr + t;
        return true;
...
class MemRecycleList
{
   ...
   void  pushFront(T* p) {
      // TODO
      if(_first == NULL){
        size_t* tem = reinterpret_cast<size_t*> (p);
        *tem = 0;
        _first = p;
      } 
      else{
        size_t* tem = reinterpret_cast<size_t*> (p);
        *tem = reinterpret_cast<size_t>(&(*_first));
        _first = p;
      }
   }
   ...
   T* getNext(T* p) const {
      // TODO
      size_t* tem = reinterpret_cast<size_t*>(p);
      T* temNext = reinterpret_cast<T*>(*tem);
      return temNext;
   }
   //
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      MemBlock<T>* tem = _activeBlock->getNextBlock();
      while(tem != NULL){
        delete _activeBlock;
        _activeBlock = tem;
        tem = _activeBlock->getNextBlock();
      }
      _activeBlock->reset();
...
private:
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      size_t m = n % R_SIZE;
      // TODO
      MemRecycleList<T>* tem = &_recycleList[m];
      while(1){
        if(tem->_arrSize == n) return tem;

        else if(tem->_nextList != NULL) tem = tem->_nextList;
        else {
          MemRecycleList<T>* temNext = new MemRecycleList<T>(n);
          tem->setNextList(temNext);
          return temNext;
        }
      }
   }
   ...
   T* getMem(size_t t) {
      ...
      MemRecycleList<T>* Recycle = getMemRecycleList(ArraySize);
      if(Recycle->_first != NULL){
        ...
        if(!(_activeBlock->getMem(t,ret))){
          size_t remainsize = downtoSizeT(_activeBlock->getRemainSize());
          if(remainsize >= S){
            _activeBlock->getMem(remainsize,ret);
            MemRecycleList<T>* temRecycle = getMemRecycleList(getArraySize(remainsize));
            temRecycle->pushFront(ret);
            #ifdef MEM_DEBUG
            cout << "Recycling " << ret << " to _recycleList[" << getArraySize(remainsize) << "]\n";
            #endif // MEM_DEBUG
          }
          MemBlock<T>* newBlock = new MemBlock<T>(0, _blockSize);
          newBlock->_nextBlock = _activeBlock;
   ...
   size_t getNumBlocks() const {
      // TODO
      size_t count = 0;
      MemBlock<T>* counter = _activeBlock;
      while(1){
        if(counter != NULL){
          count++;
          counter = counter->getNextBlock();
        }
        else return count;
      }
   }
